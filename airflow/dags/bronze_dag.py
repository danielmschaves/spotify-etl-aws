...
from spotify_etl.ingestion.bronze import DataManager, Ingestor
...
# DAG definition
@dag(
    default_args=default_args, schedule_interval="@daily", catchup=False, tags=["bronze_ingestion"]
)
def bronze_ingestion():
    ...
    @task
    def bronze_ingestion_task(file_names: list):
        # Configuration
        aws_access_key = os.getenv("AWS_ACCESS_KEY")
        aws_secret_access_key = os.getenv("AWS_SECRET_ACCESS_KEY")
        aws_region = os.getenv("AWS_REGION")
        local_path = os.getenv("BRONZE_LOCAL_PATH", "data/bronze/")
        ...
        try:
            ...
            # Create ingestor and execute
            ingestor = Ingestor(db_manager, aws_manager, data_manager, motherduck_manager)
            # Note: The ingestor.execute is already handled above, but keeping for consistency
        except Exception as e:
    ...
    bronze_playlist_task = bronze_ingestion_task(
        file_names=[
            "playlist_37i9dQZEVXbMDoHDwVN2tF_20.json",
            "playlist_37i9dQZF1DXcBWIGoYBM5M_20.json",
            "playlist_37i9dQZF1DWXRqgorJj26U_20.json",
        ]
    )
    ...
    trigger_silver_ingestion = TriggerDagRunOperator(
        ...
        conf={"file_names": "{{ task_instance.xcom_pull(task_ids='bronze_ingestion_task') }}"},
    )
