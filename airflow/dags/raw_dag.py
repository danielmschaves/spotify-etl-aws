...
from spotify_etl.ingestion.raw import SpotifyAPIClient, DataParser, DataSaver, Ingestor
...
table_path = "data/raw/"
aws_bucket_name = os.getenv("RAW_BUCKET")
...
def raw_ingestion():
    ...
    def raw_ingestion_task():
        ...
        try:
            ...
            for playlist_id in playlist_ids:
                file_name = ingestor.execute("", "playlist", playlist_id=playlist_id)
                if file_name:
    ...
    fetch_and_save_spotify_data_task = raw_ingestion_task()

    trigger_bronze_ingestion = TriggerDagRunOperator(
        task_id="trigger_bronze_ingestion",
        trigger_dag_id="bronze_ingestion",
        conf={"file_names": "{{ task_instance.xcom_pull(task_ids='raw_ingestion_task') }}"},
    )
